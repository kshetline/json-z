...
const decimalHandler = {
  name: 'Decimal',
  test: (obj, options) => !options.primitiveDecimal && big.isDecimal(obj),
  creator: big.toDecimal,
  serializer: n => big.stringify(n, { primitiveDecimal: true }, '', true)
};
...
module.exports = {
  ...
  addTypeHandler(handler) {
    // Remove any same-named handler. The new handler will go on the end, gaining priority.
  ...
  restoreStandardTypeHandlers() {
    ...
    module.exports.addTypeHandler(decimalHandler);
