...
function getDecimalType() {
  return big.getDecimalType() === 'numeric' ? 'numeric' : 'decimal';
}
...
module.exports = function parse(text, reviver, newOptions) {
  ...
  function parseNumber() {
    ...
    if (peek() === 'd') {
      read();
      return newToken(getDecimalType(), big.toDecimal(NaN));
    }
  ...
  const lexStates = {
    ...
    value() {
      ...
      switch (c) {
        ...
        case 'I':
          ...
          if (peek() === 'd') {
            read();
            return newToken(getDecimalType(), big.toDecimal(Infinity));
          }
    ...
    sign() {
      ...
      switch (c) {
        ...
        case 'I':
          ...
          if (peek() === 'd') {
            read();
            return newToken(getDecimalType(), big.toDecimal(sign * Infinity));
          }
    ...
    decimalInteger() {
      ...
      if (c === 'n') {
        read();
        return newToken('bigint', big.toBigInt(signChar + buffer));
      }
      else if (c === 'm') {
        read();
        ...
        read();
        return newToken(getDecimalType(), big.toDecimal(signChar + buffer));
      }
    ...
    string() {
      switch (c) {
        ...
        case '$':
          buffer += read();
          if (quoteType === QUOTE_BACKTICK && peek() === '{') {
