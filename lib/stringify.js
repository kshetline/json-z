...
module.exports = function stringify(value, replacer, space) {
  ...
  function serializePropertyAux(key, holder, forArray, keyStack) {
    ...
    if (typeof value === 'object') {
      return Array.isArray(value) ? serializeArray(value, keyStack) : serializeObject(value, keyStack);
    }
  ...
  function serializeObject(value, keyStack) {
    ...
    if (partial.length === 0) {
      ...
      // Type containers are not internally indented, and have a gap of either 0 or 1 space.
      if (gap === '' || gap === ' ' || itc) {
        properties = partial.join(',' + (gap ? (itc ? ' ' : gap) : ''));
        final = `{${properties}}`;
      }
      else {
        const separator = ',\n' + indent;
  ...
  function serializeArray(value, keyStack) {
    if (stack.indexOf(value) >= 0) {
      throw TypeError('Converting circular structure to JSON-Z');
    }
